...
const toInputDatetime = (dateString) => dateString.substring(0, 16).replace(' ', 'T');
...
export default function EditReq({ cats, booking, onClose, onSubmit }) {
    const [formData, setFormData] = useState({
        catId: booking.catId, customerName: booking.customer.name, customerPhone: booking.customer.phone,
        customerEmail: booking.customer.email || '', start: toInputDatetime(booking.time[0]), end: toInputDatetime(booking.time[1])
    });
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200 font-m3">
            <div className="bg-white rounded-[32px] w-full max-w-2xl shadow-2xl border-2 border-[#7838F5]/20 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-8 py-5 border-t-2 border-slate-100 bg-white flex justify-end gap-3 rounded-b-[32px]">
                    ...
                    <button form="edit-req-form" type="submit" className="px-8 py-3.5 rounded-2xl bg-[#7838F5] hover:bg-[#6025D1] text-white font-extrabold shadow-lg shadow-[#7838F5]/30">Сохранить</button>
                </div>
            </div>
        </div>,
        document.body
